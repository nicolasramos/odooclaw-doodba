...
OdooClaw now includes a SQLite-backed memory index for long-term recall and recent-note
retrieval.

It also supports an optional historical memory layer (cold storage) for scoped
historical recall, fact timeline queries, and explainability.
...
4. prompt invalidation includes memory-related sources so changes are reflected
   correctly
...
- selective source inclusion (`include_long_term`, `include_daily_notes`,
  `include_scoped`, `include_memory_notes`)
- `max_files` cap and deduped re-import behavior

This keeps the existing HOT memory behavior intact while adding optional COLD historical
...
```json
{
  "tool": "memory_save",
  "args": {
    "content": "Customer prefers concise Friday updates.",
...
```json
{
  "tool": "memory_query_facts",
  "args": {"query": "timezone", "as_of": 1712515200, "limit": 5}
}
```
...
```json
{
  "tool": "memory_debug_explain_retrieval",
  "args": {"query": "friday concise updates", "include_facts": true}
}
```

```json
{
  "tool": "memory_import_history",
  "args": {
    "dry_run": true,
...
Some older docs still refer to memory in more generic terms (for example vector/local
memory wording inherited from earlier architecture notes). The current implementation to
rely on is the SQLite-backed memory index described here.

For manual validation from Odoo Discuss, see
[Odoo Chat Memory QA Guide](ODOO_CHAT_MEMORY_QA.md).
