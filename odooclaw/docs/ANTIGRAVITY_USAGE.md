...
```bash
odooclaw auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where OdooClaw is waiting.

OdooClaw will extract the authorization code and complete the process automatically.
...
odooclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
