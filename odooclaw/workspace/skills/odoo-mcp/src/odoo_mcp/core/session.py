...
class OdooSession:
    ...
    def __init__(self, url: str, db: str, username: str, password: str):
        ...
        # Shared secret required by the mail_bot_odooclaw protected endpoints
        # (/odooclaw/call_kw_as_user, /odooclaw/reply), which default-deny unless
        # a token or IP allowlist is configured. Must match the Odoo system
        # parameter `odooclaw.reply_token`.
        token = os.environ.get("ODOOCLAW_REPLY_TOKEN", "").strip()
        if token:
            self.session.headers["X-OdooClaw-Token"] = token
