...
def create_vendor_invoice(
    ...
    _logger.info(
        f"Creating vendor invoice for partner {partner_id} with {len(lines)} lines"
    )
    return client.call_kw(
        "account.move", "create", args=[invoice_vals], sender_id=user_id
    )
...
def find_pending_invoices(
    ...
    _logger.info(
        f"Finding pending invoices: move_type={move_type}, partner_id={partner_id}"
    )
    return client.call_kw(
        "account.move",
        "search_read",
