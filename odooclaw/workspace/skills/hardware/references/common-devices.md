...
- **Parse:** Status=byte[0], Humidity=(byte[1]<<12|byte[2]<<4|byte[3]>>4)/2^20*100,
  Temp=(byte[3]&0x0F<<16|byte[4]<<8|byte[5])/2^20*200-50
- **Notes:** No register addressing — write command bytes directly (omit `register`
  param)
...
- **Config:** Write 0xF2 (humidity oversampling), 0xF4 (temp/press oversampling + mode),
  0xF5 (standby, filter)
- **Forced measurement:** Write `[0x25]` to register 0xF4, wait 40ms, read 8 bytes from
  0xF7
- **Calibration:** Read 26 bytes from 0x88 and 7 bytes from 0xE1 for compensation
  formulas
...
- **Read accel:** 6 bytes from register 0x3B (XH,XL,YH,YL,ZH,ZL) — signed 16-bit,
  default ±2g
...
- **Address:** 0x68
- **Read time:** 7 bytes from register 0x00 (seconds, minutes, hours, day, date, month,
  year) — BCD encoded
...
- **Set PWM freq:** Sleep → write prescale to 0xFE → wake. Prescale = round(25MHz /
  (4096 × freq)) - 1
...
- **Write:** Write 2-byte address + up to 64 bytes (page write), wait 5ms for write
  cycle
...
- **Read channel N:** Send `[0x01, (0x80 | N<<4), 0x00]`, result in last 10 bits of
  bytes 1-2
