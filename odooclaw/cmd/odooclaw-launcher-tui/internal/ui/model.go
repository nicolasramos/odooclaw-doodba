...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					odooclawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model odooclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []odooclawconfig.ModelConfig) {
...
func isModelValid(model odooclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *odooclawconfig.ModelConfig) {
	if model == nil {
		return
	}
