...
import (
	...
	configstore "github.com/nicolasramos/odooclaw/cmd/odooclaw-launcher-tui/internal/config"
...
func Run() error {
	...
	if cfg == nil {
		cfg = odooclawconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open odooclaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("odooclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("odooclaw", "gateway")
