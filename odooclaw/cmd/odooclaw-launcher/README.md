...
```bash
# Build
go build -o odooclaw-launcher ./cmd/odooclaw-launcher/

# Run with default config path (~/.odooclaw/config.json)
./odooclaw-launcher

# Specify a config file
./odooclaw-launcher ./config.json

# Allow LAN access
./odooclaw-launcher -public
```
...
```
Usage: odooclaw-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.odooclaw/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.odooclaw/config.json"
}
```
...
Gets the running status of the `odooclaw gateway` process.
...
Starts the `odooclaw gateway` process in the background.
...
Stops the running `odooclaw gateway` process.
...
```bash
go test -v ./cmd/odooclaw-launcher/
```
