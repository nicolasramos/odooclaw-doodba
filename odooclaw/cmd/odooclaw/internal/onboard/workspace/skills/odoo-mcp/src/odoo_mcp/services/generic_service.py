...
def get_record_summary(
    client: OdooClient, user_id: int, model: str, res_id: int
) -> dict:
    ...
    records = client.call_kw(
        model, "read", args=[[res_id]], kwargs={"fields": fields}, sender_id=user_id
    )
    if not records:
...
def get_chatter_summary(
    ...
    # Get messages
    messages = client.call_kw(
        "mail.message",
        "search_read",
        args=[[("model", "=", model), ("res_id", "=", res_id)]],
        kwargs={
    ...
    activities = client.call_kw(
        "mail.activity",
        "search_read",
        args=[[("res_model", "=", model), ("res_id", "=", res_id)]],
        kwargs={"fields": ["summary", "user_id", "date_deadline", "state"]},
        sender_id=user_id,
    )
    ...
    return {
        "latest_messages": [
            {
                "author": m.get("author_id")[1] if m.get("author_id") else "System",
                "date": m.get("date"),
        ...
        "pending_activities": [
            {
                ...
                "user": a.get("user_id")[1] if a.get("user_id") else None,
