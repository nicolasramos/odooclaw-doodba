...
def run_tests():
    ...
    try:
        res = odoo_search(
            OdooSearchSchema(model="res.partner", domain=[], limit=1, sender_id=uid)
        )
        partner_id = res[0]
        summary = odoo_get_partner_summary(
            GetPartnerSummarySchema(partner_id=partner_id, sender_id=uid)
        )
    ...
    # Inexistente
    try:
        res = odoo_get_partner_summary(
            GetPartnerSummarySchema(partner_id=999999, sender_id=uid)
        )
        if isinstance(res, dict) and "error" in res:
            print_ok("3.1 Partner Inexistente", f"Respuesta de dict: {res['error']}")
        else:
            print_fail(
                "3.1 Partner Inexistente", "No devolvió error object -> " + str(res)
            )
    except Exception as e:
        print_ok(
            "3.1 Partner Inexistente",
            f"Lanzó error correctamente. Detalle: {str(e)[:50]}...",
        )
    ...
    try:
        ...
        if "Odoo Access/ORM Error" in str(e):
            print_ok(
                "4.1 Buscar `project.task`", "PASS - Módulo 'project' no está instalado"
            )
        else:
    ...
    try:
        ...
        if "Odoo Access/ORM Error" in str(e):
            print_ok(
                "4.4 Buscar `sale.order`",
                "PASS - Módulo 'sale_management' no está instalado",
            )
        else:
    ...
    # Venta Inexistente
    try:
        res = odoo_get_sale_order_summary(
            GetSaleOrderSummarySchema(order_id=999999, sender_id=uid)
        )
        if isinstance(res, dict) and "error" in res:
        ...
        print_ok(
            "4.5 Sele Order Inexistente", f"Rechazado correctamente: {str(e)[:50]}"
        )
