...
INTERNAL=github.com/nicolasramos/odooclaw/cmd/odooclaw/internal
...
ODOOCLAW_HOME?=$(HOME)/.odooclaw
WORKSPACE_DIR?=$(ODOOCLAW_HOME)/workspace
...
## build: Build the odooclaw binary for current platform
build: generate
...
## build-all: Build odooclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install odooclaw to system and copy builtin skills
install: build
...
## uninstall: Remove odooclaw from system
uninstall:
...
## uninstall-all: Remove odooclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(ODOOCLAW_HOME)
	@echo "Removed workspace: $(ODOOCLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build odooclaw-agent odooclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build odooclaw-agent odooclaw-gateway
...
## docker-run: Run odooclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run odooclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run odooclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm odooclaw-agent

## docker-run-agent-full: Run odooclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm odooclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi odooclaw:latest odooclaw:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "odooclaw Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.odooclaw/workspace)"
