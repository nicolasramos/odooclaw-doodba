...
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	...
	// before running OdooClaw to enable Google Cloud Code Assist (Antigravity).
	clientID, clientIDOk := os.LookupEnv("GOOGLE_CLIENT_ID")
	clientSecret, clientSecretOk := os.LookupEnv("GOOGLE_CLIENT_SECRET")

	if !clientIDOk || !clientSecretOk {
		return OAuthProviderConfig{Err: ErrInvalidEnv}  // missing required env vars
	}
	return OAuthProviderConfig{
