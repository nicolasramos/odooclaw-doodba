...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.odooclaw/workspace" {
		t.Errorf("expected workspace '~/.odooclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToOdooClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToOdooClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.odooclaw/workspace" {
		t.Errorf("expected workspace '~/.odooclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToOdooClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToOdooClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.odooclaw/workspace" {
		t.Errorf("expected workspace '~/.odooclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
