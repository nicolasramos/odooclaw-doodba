...
# Upstream currently carries contaminated executable bits. Preserve executable
# mode only for real scripts with a shebang; normalize every other file.
for candidate in Path(".").rglob("*"):
    if not candidate.is_file():
        continue
    mode = candidate.stat().st_mode
    if candidate.read_bytes().startswith(b"#!"):
