...
import (
	...
	"github.com/nicolasramos/odooclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to odooclaw",
		Args:  cobra.NoArgs,
		Example: `  odooclaw migrate
  odooclaw migrate --from openclaw
  odooclaw migrate --dry-run
  odooclaw migrate --refresh
  odooclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.odooclaw)")
